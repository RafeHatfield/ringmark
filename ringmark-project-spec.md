...
The REST API enables LLM/MCP clients and future integrations to interact with workshop data without a browser session. It uses the same underlying data model as the admin UI but is accessed via HTTP with a long-lived API key rather than a Supabase Auth session.

### Authentication

All object endpoints require:
```
Authorization: Bearer <RINGMARK_API_KEY>
```

The API key is set via the `RINGMARK_API_KEY` environment variable. The server uses `crypto.timingSafeEqual` to prevent timing-based key guessing. If the env var is missing, the server returns 401 (fail-closed).
...
| GET | /api/v1/objects/:id | Get single object (UUID or workshop ID) |
| PATCH | /api/v1/objects/:id | Partial update; whitelist only; is_published toggles publish state |
...
The `:id` parameter accepts either a UUID or a workshop ID (e.g. `RH1`, `RH1-2`). Case-insensitive.

### Self-Documenting System

`lib/api-schemas.ts` is the single source of truth. Zod schemas annotated with `.openapi()` drive both request validation (used in route handlers) and the OpenAPI 3.1 spec (generated by `lib/api-spec.ts`). Adding an endpoint means updating both the route handler and `api-spec.ts`.

### Security Notes

- API routes use a service role Supabase client (`lib/supabase/service.ts`) that bypasses RLS. Authorization is enforced by `verifyApiKey()` and by scoping all queries to the single account via `getAccount()`.
- `public_slug` is never writable via the API. It is intentionally excluded from `PatchObjectSchema`.
- `account_id` is always resolved server-side from the account table, never accepted from the client.
- Private fields (`private_notes`, `location_text`) are included in API responses (this is an owner-facing API, not the public story page). The public page at `/p/[slug]` continues to exclude them.
...
Runs 58 Playwright integration tests in `e2e/api.spec.ts` against the live dev server (`http://localhost:3000`). Tests cover auth rejection, CRUD on objects, child creation with flat-numbered IDs, filtering/search, and cleanup.
